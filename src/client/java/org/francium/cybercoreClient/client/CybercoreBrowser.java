...
/**
 * {@link MCEFBrowser} that knows when a frame actually reached the texture.
 *
 * <p>MCEF exposes no paint callback, and its accelerated path drops frames silently - empty or 1x1
 * damage, and everything with partial damage after a size change. The texture keeps whatever frame
 * was copied last, so one silently dropped frame at the wrong moment leaves the previous page on
 * screen for good. Subclassing is the fork's own escape hatch for this ("in case a mod wants to
 * extend MCEFBrowser and override the repaint logic"), and a delivery timestamp is all the mod
 * needs: the overlay gate can then wait for a frame that verifiably arrived, and ask again when
 * none does, instead of assuming the one it asked for made it.
 */
final class CybercoreBrowser extends MCEFBrowser {

    /** When a frame last made it into the texture. Written on the render thread. */
    private volatile long frameDeliveredNanos = 0;

    /**
     * Mirror of MCEFBrowser's private lastWidth/lastHeight, fed the same stream, so this class can
     * replay the superclass's accept-or-drop decision without reflection.
     */
    private int mirrorWidth = 0;
    private int mirrorHeight = 0;
    ...
    /** Nanotime of the last frame that was actually copied, 0 before the first. */
    long frameDeliveredNanos() {
        return frameDeliveredNanos;
    }
    ...
    public void onPaint(CefBrowser browser, boolean popup, Rectangle[] dirtyRects,
        ...
        // The software path copies whenever there is damage on the main surface.
        if (!popup && dirtyRects.length > 0) {
            frameDeliveredNanos = System.nanoTime();
        }
    }
    ...
    public void onAcceleratedPaint(CefBrowser browser, boolean popup, Rectangle[] dirtyRects,
        ...
        if (acceleratedFrameAccepted(popup, dirtyRects, info)) {
            frameDeliveredNanos = System.nanoTime();
        }
    }

    /** The superclass's filter, replayed: only frames that pass it reach the texture. */
    private boolean acceleratedFrameAccepted(boolean popup, Rectangle[] dirtyRects,
                                             CefAcceleratedPaintInfo info) {
        if (popup || dirtyRects.length == 0) {
            return false;
        }
        ...
        if (mirrorWidth != info.width || mirrorHeight != info.height) {
            Rectangle rect = dirtyRects[0];
            if (rect.width == info.width && rect.height == info.height
                    && rect.x == 0 && rect.y == 0) {
                mirrorWidth = info.width;
                mirrorHeight = info.height;
            } else {
                return false;
            }
        }
